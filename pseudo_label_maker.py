...
def validate(valid_loader, device, model, opt):
    ...
    for data_i in tqdm(valid_loader):
        images_val = data_i[1].to(device)
        labels_val = data_i[2].to(device)
        label_da, label_ll = labels_val
        filename = data_i[0]
        ...
        if opt.soft:
            ...
            for k in range(label_da.shape[0]):
            ...
            if opt.flip:
                ...
                flip_out['out'] = F.interpolate(sm(flip_out['out']), size=images_val.size()[2:], mode='bilinear',
                                                align_corners=True)
                out['out'] = F.interpolate(sm(out['out']), size=images_val.size()[2:], mode='bilinear',
                                           align_corners=True)
            ...
            for k in range(labels_val.shape[0]):
                ...
                Image.fromarray(pseudo_rgb[k].permute(1, 2, 0).cpu().numpy().astype(np.uint8)).save(
                    os.path.join(ori_LP, name[:-4] + '_color.png'))
                np.save(os.path.join(ori_LP, name.replace('.png', '_conf.npy')),
                        confidence[k, 0].cpu().numpy().astype(np.float16))
